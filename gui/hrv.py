...
class HRVProcessor(QObject):
    ...
    def compute_hrv(self):
        ...
        try:
            ...
            # Store RRI — this is what the plot windows read
            self._rri_ms = np.diff(peaks["PPG_Peaks"]) / self.sampling_rate * 1000

            self.hrv_computed.emit({
                "rmssd": rmssd, "sdnn": sdnn, "mean_rr": mean_rr, "pnn50": pnn50,
                "lf": lf, "hf": hf, "lf_hf": lf_hf, "sd1": sd1, "sd2": sd2,
            })
        except Exception as e:
